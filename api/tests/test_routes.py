...
@patch('api.utils.authentication.validate_token_and_get_user_id', return_value='123e4567-e89b-12d3-a456-426614174000')
...
@patch('api.services.categories_service.get_categories', return_value=[{'id': 1, 'name': 'Fiction'}])
def test_get_categories_success(mock_get, client):
    resp = client.get('/api/categories')
    assert resp.status_code == 200
    assert isinstance(resp.json, list)

@patch('api.services.categories_service.get_categories', side_effect=Exception('DB error'))
