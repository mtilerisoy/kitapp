...
def test_books_repository_fetch_paginated_books_success(mock_db_client):
    ...
    mock_db_client.table.return_value.select.return_value.order.return_value.range.return_value.execute.return_value = (
        [None, [{"id": 1, "title": "Book"}]],
        None,
    )
...
def test_categories_repository_fetch_all_success(mock_db_client):
    ...
    mock_db_client.table.return_value.select.return_value.order.return_value.execute.return_value = (
        [None, [{"id": 1, "name": "Fiction"}]],
        None,
    )
    cats = repo.fetch_all()
